...
class TestOdometry(unittest.TestCase):
    ...
    def assertOdomResultAlmostEqual(self, result0, result1):
        ...
        self.assertPoseAlmostEqual(pose0, pose1, delta_v=0.02, delta_q=3, msg=msg%'estimated poses')
        self.assertArrayAlmostEqual(bias_sds0, bias_sds1, delta=0.1, ord=np.inf, msg=msg%'error estimates')
        self.assertAlmostEqual(scale_sd0, scale_sd1, delta=0.11, msg=msg%'scale error estimate')
    ...
    def test_rotating_object(self, inputs=None, results=None):
        ...
        for i, (img, cam_obj_v, cam_obj_q) in enumerate(inputs):
            ...
            if res[0] is not None and 0:
                tools.plot_quats((prior.quat, res[0].quat))

            if i > 1 and 0:
                self.assertIsNotNone(res[0], msg='failed to get pose estimate')

                # TODO: fix odometry.py, now must accept huge error (90deg)
                self.assertPoseAlmostEqual(prior, res[0], delta_v=0.1, delta_q=90,
