...
class KeypointAlgo(AlgorithmBase):
    ...
    MAX_FEATURES = 1000         # upper limit of features to calculate from an image
    ...
    DEF_RANSAC_ERROR = 8  # default 8.0, for ORB: use half the error given here
    ...
    def solve_pnp(self, orig_sce_img, outfile, feat=ORB, use_feature_db=False, adjust_sc_rot = False,
        ...
        for i in range(self.MAX_SCENE_SCALE_STEPS):
            try:
                ...
                if np.isclose(sce_img_sc, 1):
                    ...
                    sce_img = cv2.resize(orig_sce_img, None, fx=sce_img_sc, fy=sce_img_sc, interpolation=cv2.INTER_AREA)
                ...
                if verbose > 0 and (not BATCH_MODE or DEBUG):
                    logger.info('matches: %s/%s'%(len(matches), min(len(sce_kp), len(ref_kp))))
                ...
                if verbose > 0:
                    logger.info('inliers: %s/%s, ' % (inlier_count, len(matches)))
        ...
        if verbose > 0 and (not BATCH_MODE or DEBUG):
            ...
            logger.info('repr-err: %.2f, rel-rot-err: %.2f°, dist-err: %.2f%%, lat-err: %.2f%%, shift-err: %.1fm'%(
                rp_err,
    ...
    def render_ref_img(self, ref_img_sc):
        ...
        ref_img = cv2.resize(ref_img, None, fx=ref_img_sc, fy=ref_img_sc, interpolation=cv2.INTER_AREA)
...
if __name__ == '__main__':
    ...
    logger.info('s/c pose adjusted shift error: %f' % sm.calc_shift_err())
    ...
    logger.info('ast pose adjusted shift error: %f' % sm.calc_shift_err())
